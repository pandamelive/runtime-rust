...
RUN MOLD_VERSION=$(curl -s https://api.github.com/repos/rui314/mold/releases/latest | grep -oP '"tag_name": "\K[^"]+') \
    && curl -sL "https://github.com/rui314/mold/releases/download/${MOLD_VERSION}/mold-${MOLD_VERSION}-x86_64-linux.tar.gz" -o /tmp/mold.tar.gz \
